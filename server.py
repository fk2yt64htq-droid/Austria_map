...
def get_stats():
    ...
    for row in rows:
        ...
        if timestamp_str:
            try:
                if now - datetime.fromisoformat(timestamp_str) > timedelta(hours=1.5):
                    is_expired = True
            except: pass
...
def save_feedback():
    ...
    # 2. ПРИМУСОВА ВІДПРАВКА БЕЗ УМОВ
    message_text = f"💡 Новий відгук від {first_name} (@{username}): {text}"
    try:
        requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", 
                      json={"chat_id": ADMIN_CHAT_ID, "text": message_text}, timeout=10)
